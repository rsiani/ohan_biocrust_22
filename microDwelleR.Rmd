...
ggplot(alpha_frame %>% 
       ...
       aes(x = type,
           ...
           fill = type,
           title = type)) +
  geom_jitter(aes(shape = site), size = 7.5) +
  scale_shape_manual(values = c(21, 23)) +
  geom_boxplot(outlier.shape = NA, alpha = .1, color = "#757575") +
  ...
  facet_wrap(
    ~ name,
    scales = "free", ncol = 2) -> a

# save all in a pdf

cairo_pdf("OUT/alphaDiversity_united.pdf",
    width = 16,
    height = 16,
    onefile = T)
a
